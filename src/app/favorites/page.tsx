...
export default function FavoritesPage() {
  ...
  useEffect(() => {
    fetchFavorites();
    const savedView = localStorage.getItem("favoritesViewMode") as
      | "grid"
      | "list";
  ...
  const handleSelectSuggestion = async (suggestion: any) => {
    ...
    try {
      ...
      const year =
        yearClaim?.mainsnak?.datavalue?.value?.time?.match(/\d{4}/)?.[0] || "";
      ...
      if (creatorId) {
        ...
        creatorName =
          creatorData.entities?.[creatorId]?.labels?.en?.value || "";
      }
      ...
      try {
        ...
        const entityData = await entityRes.json();
        const wikiTitle =
          entityData.entities?.[suggestion.id]?.sitelinks?.enwiki?.title;
  ...
  useEffect(() => {
    ...
    const debounceId = setTimeout(async () => {
      ...
      try {
        ...
        // Filter out nulls
        const filteredSuggestions = suggestionsWithImages.filter(
          (s) => s !== null,
        );
  ...
  const handleSelectCreatorSuggestion = async (suggestion: any) => {
    ...
    try {
      ...
      const birthYear =
        birthYearClaim?.mainsnak?.datavalue?.value?.time?.match(/\d{4}/)?.[0] ||
        "";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        !formData.creatorMode &&
        (!editingFavorite ||
          editingFavorite.title !== formData.title ||
          editingFavorite.type !== formData.type)
      ) {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder={
                      formData.creatorMode ? t("optional" as any) : undefined
                    }
                  />
                </div>
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder={
                      formData.creatorMode
                        ? t("whyDoYouFollowThis" as any) ||
                          "Bu kişiyi neden takip ediyorsun?"
                        : t("whyDoYouLoveThis" as any) || "Neden seviyorsun?"
                    }
                  />
